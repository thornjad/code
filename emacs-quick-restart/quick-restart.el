;;; quick-restart.el --- Restart emacs from within emacs  -*- lexical-binding: t; -*-
;;
...
;; is forked from restart-emacs by Iqbal Ansari.
;;

;;; Code:
...
(defgroup quick-restart nil
	"Customization options for quick-restart"
	:group 'tools
	:prefix "quick-restart-")

(defcustom quick-restart-daemon-with-tty-frames-p nil
	"Restart Emacs daemon even if it has tty frames.

Currently `quick-restart' cannot restore such frames, it just
notifies the user once the daemon has restarted"
	:type 'boolean
	:group 'quick-restart)

(defcustom quick-restart-restore-frames nil
...
(defun quick-restart--string-join (strings &optional separator)
...
(defun quick-restart--user-error (format &rest args)
...
parameters.  IGNORED are ignored."
	(when (processp (cdr current))
		(let ((window (frame-selected-window (process-get (cdr current) 'frame))))
			(cons 'quick-restart-file (buffer-file-name (window-buffer window))))))

(defun quick-restart--notify-connection-instructions (tty filename)
...
(defun quick-restart--frameset-tty-filter (tty filtered parameters saving)
...
PARAMETERS and SAVING."
	(when (cdr tty)
		(run-at-time 0.5
								 nil
								 (apply-partially 'quick-restart--notify-connection-instructions
																	(cdr tty)
																	(cdr (assoc 'quick-restart-file filtered)))))
	(frameset-filter-tty-to-GUI tty filtered parameters saving))

(defun quick-restart--restore-frames-using-desktop (file)
	...
	(let* (desktop-file-modtime
				 ...
				 (frameset-filter-alist (append '((tty . quick-restart--frameset-tty-filter))
...
(defun quick-restart--frame-restore-args ()
	...
	(when (and (locate-library "frameset")
						 ...
						 ;; the frames in daemon mode. Also ignore the `quick-restart-restore-frames'
						 ;; configuration since restarting the daemon without restoring frames
						 ;; doesn't really help
						 (or (daemonp)
								 (and quick-restart-restore-frames
											(not (bound-and-true-p desktop-save-mode)))))
		(list "--quick-restart-desktop"
					(quick-restart--save-frames-using-desktop))))

(defun quick-restart--start-gui-using-sh (&optional args)
...
ARGS is the list arguments with which Emacs should be started"
	(call-process "sh" nil
								0 nil
								"-c" (format "%s %s &"
														 (shell-quote-argument (quick-restart--get-emacs-binary))
														 (quick-restart--string-join (mapcar #'shell-quote-argument
																																 args)
																												 " "))))

(defun quick-restart--start-emacs-in-terminal (&optional args)
...
(defun quick-restart--daemon-using-sh (&optional args)
...
current instance"
	(call-process "sh" nil
								...
								"-c" (format "%s --daemon=%s %s &"
														 (shell-quote-argument (quick-restart--get-emacs-binary))
														 server-name
														 (quick-restart--string-join (mapcar #'shell-quote-argument args)
																												 " "))))

(defun quick-restart--ensure-can-restart ()
	"Ensure we can restart Emacs on current platform."

	(when (and (daemonp)
						 (not (locate-library "frameset")))
		(quick-restart--user-error "Cannot restart Emacs daemon on versions before 24.4"))

	(when (and (daemonp)
						 (delq nil (mapcar (lambda (frame)
																 (frame-parameter frame 'tty))
															 (frame-list)))
						 (not quick-restart-daemon-with-tty-frames-p)
						 (not (yes-or-no-p "Current Emacs daemon has tty frames, `quick-restart' cannot restore them, continue anyway? ")))
		(quick-restart--user-error "Current Emacs daemon has tty frames, aborting `quick-restart'.
Set `quick-restart-with-tty-frames-p' to non-nil to restart Emacs irrespective of tty frames")))

(defun quick-restart--launch-other-emacs (arguments)
	"Launch another Emacs session with ARGUMENTS according to current platform."
	(apply (cond ((daemonp) #'quick-restart--daemon-using-sh)
							 ((display-graphic-p) #'quick-restart--start-gui-using-sh)
							 (t #'quick-restart--start-emacs-in-terminal))
				 ;; Since this function is called in `kill-emacs-hook' it cannot accept
				 ;; direct arguments the arguments are let-bound instead
				 (list arguments)))

(defun quick-restart--translate-prefix-to-args (prefix)
...
(defun quick-restart--guess-startup-directory-using-proc ()
...
(defun quick-restart--guess-startup-directory-using-lsof ()
...
(defun quick-restart--guess-startup-directory-using-buffers ()
...
(defun quick-restart--guess-startup-directory-from-env ()
	"Guess the startup directory for current Emacs session from USERPROFILE or HOME."
	(or (getenv "HOME")
			(getenv "USERPROFILE")))

(defun quick-restart--guess-startup-directory ()
...
the HOME or USERPROFILE (only applicable on Window) environment variable and
	...
	(or (quick-restart--guess-startup-directory-using-proc)
			(quick-restart--guess-startup-directory-using-lsof)
			(quick-restart--guess-startup-directory-using-buffers)
			(quick-restart--guess-startup-directory-from-env)
			default-directory))
...
;;;###autoload
(defun quick-restart-handle-command-line-args (&rest ignored)
	"Handle the --quick-restart-desktop command line argument.

The value of the argument is the desktop file from which the frames should be
restored.  IGNORED are ignored."
	(quick-restart--restore-frames-using-desktop (pop command-line-args-left)))

;;;###autoload
(add-to-list 'command-switch-alist '("--quick-restart-desktop" . quick-restart-handle-command-line-args))

;;;###autoload
(defun quick-restart (&optional args)
	"Restart Emacs.
...
with which Emacs should be restarted."
	...
	(quick-restart--ensure-can-restart)
	;; We need the new emacs to be spawned after all kill-emacs-hooks
	;; have been processed and there is nothing interesting left
	(let* ((default-directory (quick-restart--guess-startup-directory))
				 (translated-args (if (called-interactively-p 'any)
															(quick-restart--translate-prefix-to-args args)
														args))
				 (restart-args (append translated-args
															 ;; When Emacs is started with a -Q
															 ;; quick-restart's autoloads would not be present
															 ;; causing the the --quick-restart-desktop
															 ;; argument to be unhandled
															 (unless (member "-Q" translated-args)
																 (quick-restart--frame-restore-args))))
				 (kill-emacs-hook (append kill-emacs-hook
																	(list (apply-partially #'quick-restart--launch-other-emacs
																												 restart-args)))))
		(save-buffers-kill-emacs)))

(provide 'quick-restart)
