...
makecoffee will transform ` *` inside block comments into the more
CoffeeScript-like `#`.
...
The CoffeeScript code `on = 2` will produce errors. As such, makecoffee will
...
As such, makecoffee will throw a warning when `==` is used, but it will behave
like `===`.
...
undefined = 2`. While this is undesirable and never recommended, makecoffee
...
CoffeeScript and CoffeeScriptRedux will both choke on this. makecoffee will
...
as its parameters (eg, `(undefined) ->`). makecoffee will simply strip it out
...
of CoffeeScript v1.8.0). To work around this, makecoffee inserts an empty
...
makecoffee takes the same convention into consideration.
...
probably would in JavaScript. makecoffee will consolidate empty `case` clauses
